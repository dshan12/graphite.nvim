...
function M.setup(opts)
	...
	if vim.fn.executable(vim.g.graphite_executable) ~= 1 then
		vim.notify(
			string.format("graphite.nvim: '%s' not found in PATH. Install Graphite CLI from https://graphite.dev",
				vim.g.graphite_executable),
			vim.log.levels.WARN
		)
	end
	...
	-- Diff commands
	vim.api.nvim_create_user_command("GraphiteDiffApply", function(opts)
		require("graphite.commands.diff").apply(opts.fargs)
	end, { nargs = "*" })
	vim.api.nvim_create_user_command("GraphiteDiffReset", function(opts)
		require("graphite.commands.diff").reset(opts.fargs)
	end, { nargs = "*" })
